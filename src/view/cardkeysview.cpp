...
static void updateTreeWidgetItem(CardKeysWidgetItem *item, const Card *card, const KeyPairInfo &keyInfo, const Subkey &subkey, CardKeysView::Options options)
{
    Q_ASSERT(item);
    // slot
...
static std::vector<QAction *> actionsForCardSlot(const Card *card, const KeyPairInfo &keyInfo, const Subkey &subkey)
{
    switch (card->appType()) {
...
static auto getSortedSubkeys(const std::string &keyGrip)
{
    ...
    // sort subkeys to ensure a stable list order
    ...
    , mOptions{options}
{
    ...
    connect(mTreeWidget, &QAbstractItemView::doubleClicked, this, [this]() {
    ...
    for (int slotIndex = 0; slotIndex < int(cardKeyInfos.size()); ++slotIndex) {
        ...
        if (subkeys.empty()) {
            if (items.empty()) {
                ...
                updateTreeWidgetItem(items.front(), card, keyInfo, Subkey{}, mOptions);
            ...
            if (items.empty()) {
                ...
                updateTreeWidgetItem(items.front(), card, keyInfo, subkeys.front(), mOptions);
                ...
                while (i < int(items.size()) && s < int(subkeys.size())) {
                    ...
                    if (itemVsSubkey < 0) {
                        ...
                        updateTreeWidgetItem(item, card, keyInfo, subkey, mOptions);
                        ++i;
                        ++s;
                    } else {
                ...
                while ((insertIndex < mTreeWidget->topLevelItemCount()) //
                       && (static_cast<CardKeysWidgetItem *>(mTreeWidget->topLevelItem(insertIndex))->slotIndex())) {
                    ++insertIndex;
                }
                insertIndex -= s;
...
void CardKeysView::insertTreeWidgetItem(const Card *card, int slotIndex, const KeyPairInfo &keyInfo, const Subkey &subkey, int index)
{
    ...
    auto actionsButton = createActionsButton(card, keyInfo, subkey);
...
{
    const auto actions = actionsForCardSlot(card, keyInfo, subkey);
